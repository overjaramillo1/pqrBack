...
def main(event, context):
    ...
    cur = conn.cursor()
    try:
        cur.execute("select * from dbo.sp_get_requests_by_filter(%s::date,%s::date,%s::bigint,%s::character varying,%s::character varying,%s::int[],%s::int[],%s::int[],%s::character varying[],%s::int,%s::int)",
                    (data['i_date'],
                     data['f_date'],
                     data['filing_number'],
                     data['doc_id'],
