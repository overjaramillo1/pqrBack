...
def main(event, context):
    ...
    cur = conn.cursor()
    try:
        cur.execute("select * from dbo.sp_createapplicanttype(%s::character varying,%s::character varying,%s::int,%s::character varying,%s::date,%s::character varying,%s::date)", 
                (data['applicant_type_name'],
                 data['applicant_type_description'],
                 1,
                 username,
                 current_date,
                 username,
