...
def main(event, context):
    ...
    cur = conn.cursor()
    try:
        cur.execute("select * from dbo.sp_createrequesttype(%s::character varying,%s::character varying,%s::int,%s::character varying,%s::date,%s::character varying,%s::date)", 
                (data['request_type_name'],
                 data['request_type_description'],
                 1,
                 username,
                 current_date,
                 username,
