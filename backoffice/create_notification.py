...
def main(event, context):
    ...
    cur = conn.cursor()
    try:
        cur.execute("select * from dbo.sp_create_notification(%s::character varying,%s::character varying,%s::character varying[],%s::int,%s::int,%s::int,%s::character varying,%s::date,%s::character varying,%s::date)", 
